...
float *aumentodepos(float, float, float, int );
...
int main () {
	...
	for(int i =0; i<num;i++){
		...
		float *p = aumentodepos(velocidades[i], posiciones[i], paso, i);
		velocidades[i+1]= *p;
		posiciones[i+1] = *(p+1);
...
float *aumentodepos(float vel, float pos, float h, int lugar){
