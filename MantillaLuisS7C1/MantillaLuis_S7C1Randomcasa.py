...
cuadradoPuntos = np.random.rand(1000,2)*30.5
cuadradoPuntosX = cuadradoPuntos[:,0]
cuadradoPuntosY = cuadradoPuntos[:,1]
l=0
j=0
while(l<1000):
	for i in range(100):
		caminata = np.random.normal(0,0.03,2)
		cuadradoPuntosX[j]+=caminata[0]	
		cuadradoPuntosY[j]+=caminata[1]
		if(cuadradoPuntosX[j]>30.5):
			cuadradoPuntosX = np.delete(cuadradoPuntosX,j)
			cuadradoPuntosY = np.delete(cuadradoPuntosY,j)
			j+=-1
			break
		elif(cuadradoPuntos[j,0]<0):
			cuadradoPuntosX = np.delete(cuadradoPuntosX,j)
...
print("Quedaron en la simulacion abierta ", j, " particulas")
plt.figure()
plt.scatter(cuadradoPuntosX,cuadradoPuntosY,c='r',s=2 )
plt.title("Abiertas")
...
while(i<10000):
	cafe = np.random.rand(2)*30.3-15.17
	if(cafe[0]**2 + cafe[1]**2 <230):
		cafex = np.append(cafex,[cafe[0]])
		cafey = np.append(cafey,[cafe[1]])
		i=i+1

while(j<100):
	crema = np.random.rand(2)*2.8284-1.414
	if(crema[0]**2 + crema[1]**2 <2):
		cremax = np.append(cremax,[crema[0]])
		cremay = np.append(cremay,[crema[1]])
		j=j+1


plt.figure()
plt.scatter(cafex,cafey,s=2,c='sienna' )
plt.scatter(cremax,cremay,s=2,c='peachpuff')
...
for j in range(10000):
	...
	while(i<1000):	
		caminata = np.random.normal(0,2.5,2)
		if((cafex[j]+caminata[0])**2+ (cafey[j] +caminata[1])**2 < 230):
			cafex[j] = cafex[j]+caminata[0]	
			cafey[j] = cafey[j] +caminata[1]
			i=i+1
...
for j in range(100):
	...
	while(i<1000):	
		caminata = np.random.normal(0,2.5,2)
		if((cremax[j]+caminata[0])**2+ (cremay[j] +caminata[1])**2 < 230):
			cremax[j] = cremax[j]+caminata[0]	
			cremay[j] = cremay[j] +caminata[1]
			i=i+1
...
plt.scatter(cafex,cafey,s=2,c='sienna' )
plt.scatter(cremax,cremay,s=2,c='peachpuff')
plt.title("Cafe con crema Final")
